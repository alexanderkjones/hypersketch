import { ArcRotateCamera, Vector3, Color3, MeshBuilder, GizmoManager, CubeTexture, PBRMetallicRoughnessMaterial, HighlightLayer, KeyboardEventTypes, PointerEventTypes, PointerDragBehavior, VertexBuffer} from "@babylonjs/core";
...
import { AluminumCube, AluminumSphere, AluminumPyramid } from '../babylonjs/Objects'
	      ...
	      switch (pointerInfo.type) {
	        case PointerEventTypes.POINTERDOWN:
	        	...
	        	if(pointerInfo.pickInfo.pickedMesh){
	        		...
	        		if(['ground', 'BackgroundPlane', 'BackgroundSkybox'].includes(pointerInfo.pickInfo.pickedMesh.name)){
					...
					this.activeMeshPositions = this.activeMesh.getVerticesData(VertexBuffer.PositionKind);
					...
					for(let i=0; i<this.activeMeshIndices.length; i++){
						let vertLocal = Vector3.FromArray(this.activeMeshPositions, 3 * this.activeMeshIndices[i])
						let vertGlobal = Vector3.TransformCoordinates(vertLocal, this.activeMesh.getWorldMatrix())
	        ...
	        case PointerEventTypes.POINTERMOVE:
	          ...
	          if(['ground', 'BackgroundPlane', 'BackgroundSkybox'].includes(pickInfo.pickedMesh.name)){
	          	return;
	          }
		...
		this.scene.onKeyboardObservable.add((kbInfo) => {
			...
			switch (kbInfo.type) {
				...
				case KeyboardEventTypes.KEYUP:
					if(['Meta', 'Control', 'Alt', 'Shift'].includes(kbInfo.event.key)){
						this.activeModifierKeys.splice(this.activeModifierKeys.indexOf(kbInfo.event.key), 1);
						return;
					}
				break;
			}
