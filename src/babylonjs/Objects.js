...
export const AluminumCube = (scene, size) => {
	...
	let mesh = MeshBuilder.CreateBox("aluminum_cube_"+uuidv4(), { size: size }, scene);
	mesh.material = mat;
...
export const AluminumPyramid = (scene, size) =>{
	let mat = AluminumPBRMaterial(scene);
	let mesh = MeshBuilder.CreatePolyhedron("aluminum_pyramid_"+uuidv4(), {type: 8, size: size}, scene);
	mesh.material = mat;
	mesh.position.x += size;
	return mesh;
}
...
export const AluminumSphere = (scene, diameter) =>{
	let mat = AluminumPBRMaterial(scene);
	let mesh = MeshBuilder.CreateSphere("aluminum_sphere_"+uuidv4(), {diameter: diameter}, scene);
	mesh.material = mat;
